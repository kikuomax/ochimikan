 ...
 * @return {boolean}
 *     Whether `obj` is an `ActorScheduler`. `false` if `obj` is not specified.
 */
 ...
 *
 * Never checks if `obj` actually can be an `ActorScheduler` because this method
 ...
 *
 * Throws an exception
 * - if `actor` is not specified
 * - or if `actor` is not an `Actor`
 *
 * @method schedule
 * @param actor {Actor}
 ...
 *
 * Executed `Actor`s will be deleted from the queue of this `ActorScheduler`.
 *
 * @method run
 */
